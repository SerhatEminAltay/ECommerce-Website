document.addEventListener('DOMContentLoaded', function() {
    ...
    function filterProducts() {
        var selectedColors = Array.from(document.querySelectorAll('.filter input[type="checkbox"]:checked'))
            .filter(function(checkbox) {
                return checkbox.parentElement.parentElement.previousElementSibling.textContent === 'Renk';
            })
        ...
        var selectedPrices = Array.from(document.querySelectorAll('.filter input[type="checkbox"]:checked'))
            .filter(function(checkbox) {
                return checkbox.parentElement.parentElement.previousElementSibling.textContent === 'Fiyat';
            })
            .map(function(checkbox) {
                return checkbox.id;
            });
...
function redirectToProduct(category, color, id, name, price, image) {
    window.location.href = `../Product/Product.html?category=${category}&color=${color}&id=${id}&name=${name}&price=${price}&image=${image}`;
}
